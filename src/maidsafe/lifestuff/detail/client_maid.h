...
template<Product Product>
ClientMaid<Product>::ClientMaid(Session& session, const Slots& slots)
...
template<Product Product>
void ClientMaid<Product>::CreateUser(const Keyword& keyword,
                                     const Pin& pin,
                                     const Password& password,
                                     const boost::filesystem::path& storage_path,
                                     ReportProgressFunction& report_progress) {
...
template<Product Product>
void ClientMaid<Product>::LogIn(const Keyword& keyword,
                                const Pin& pin,
                                const Password& password,
                                ReportProgressFunction& report_progress) {
  try {
...
template<Product Product>
void ClientMaid<Product>::LogOut() {
  //  client_controller_->StopVault(  );  parameters???
  UnMountDrive();
}

template<Product Product>
void ClientMaid<Product>::MountDrive(const boost::filesystem::path&) {
  user_storage_.MountDrive(*storage_, session_);
  return;
}

template<Product Product>
void ClientMaid<Product>::UnMountDrive() {
  user_storage_.UnMountDrive(session_);
  return;
}

template<Product Product>
void ClientMaid<Product>::ChangeKeyword(const Keyword& old_keyword,
                                        const Keyword& new_keyword,
                                        const Pin& pin,
                                        const Password& password,
                                        ReportProgressFunction& report_progress) {
...
template<Product Product>
void ClientMaid<Product>::ChangePin(const Keyword& keyword,
                                    const Pin& old_pin,
                                    const Pin& new_pin,
                                    const Password& password,
                                    ReportProgressFunction& report_progress) {
...
template<Product Product>
void ClientMaid<Product>::ChangePassword(const Keyword& keyword,
                                         const Pin& pin,
                                         const Password& new_password,
                                         ReportProgressFunction& report_progress) {
...
template<Product Product>
boost::filesystem::path ClientMaid<Product>::mount_path() {
  return user_storage_.mount_path();
}

template<Product Product>
boost::filesystem::path ClientMaid<Product>::owner_path() {
  return user_storage_.owner_path();
}

template<Product Product>
const Slots& ClientMaid<Product>::CheckSlots(const Slots& slots) {
...
template<Product Product>
void ClientMaid<Product>::PutSession(const Keyword& keyword, const Pin& pin, const Password& password) {
...
template<Product Product>
void ClientMaid<Product>::DeleteSession(const Keyword& keyword, const Pin& pin) {
...
template<Product Product>
void ClientMaid<Product>::GetSession(const Keyword& keyword, const Pin& pin, const Password& password) {
  Mid::name_type mid_name(Mid::GenerateName(keyword, pin));
  auto mid_future(maidsafe::nfs::Get<Mid>(*storage_, mid_name));
  Mid mid(*mid_future.get());
  passport::EncryptedTmidName encrypted_tmid_name(mid.encrypted_tmid_name());
...
template<Product Product>
void ClientMaid<Product>::JoinNetwork(const Maid& maid) {
...
template<Product Product>
void ClientMaid<Product>::RegisterPmid(const Maid& maid, const Pmid& pmid) {
...
template<Product Product>
void ClientMaid<Product>::UnregisterPmid(const Maid& maid, const Pmid& pmid) {
...
template<Product Product>
void ClientMaid<Product>::UnCreateUser(bool fobs_confirmed, bool drive_mounted) {
  if (fobs_confirmed) {
...
template<Product Product> template<typename Fob>
void ClientMaid<Product>::PutFob(const Fob& fob) {
...
template<Product Product> template<typename Fob>
void ClientMaid<Product>::DeleteFob(const typename Fob::name_type& fob_name) {
  ReplyFunction reply([this] (maidsafe::nfs::Reply reply) {
                        if (!reply.IsSuccess()) {
...
template<Product Product> template<typename Fob>
Fob ClientMaid<Product>::GetFob(const typename Fob::name_type& fob_name) {
  std::future<Fob> fob_future(maidsafe::nfs::Get<Fob>(*storage_, fob_name));
  return fob_future.get();
}

template<Product Product>
void ClientMaid<Product>::PutFreeFobs() {
  ReplyFunction reply([this] (maidsafe::nfs::Reply reply) {
                        if (!reply.IsSuccess()) {
                          ThrowError(VaultErrors::failed_to_handle_request);
                        }
...
template<Product Product>
void ClientMaid<Product>::PutPaidFobs() {
  ReplyFunction reply([this] (maidsafe::nfs::Reply reply) {
                        if (!reply.IsSuccess()) {
                          ThrowError(VaultErrors::failed_to_handle_request);
                        }
...
template<Product Product>
void ClientMaid<Product>::PublicKeyRequest(const NodeId& node_id, const GivePublicKeyFunctor& give_key) {
  if (storage_) {
...
class ClientMaid<Product::kSureFile> {
 ...
 private:
  ...
  void DeleteSession(const Keyword& keyword, const Pin& pin) {
    ...
    serialised_mid = storage_->Get(mid_name);
